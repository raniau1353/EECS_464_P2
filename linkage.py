...
class FiveBar:
    ...
    def get_intersections(self, x0, z0, r0, x1, z1, r1):
        ...
        # non intersecting
        if d > r0 + r1 :
            sys.exit("ERROR: Non-intersecting circles")
        # One circle within other
        if d < abs(r0-r1):
            sys.exit("ERROR: One circle within another")
        # coincident circles
        if d == 0 and r0 == r1:
            sys.exit("ERROR: Coincident circles")
        else:
            a=(r0**2-r1**2+d**2)/(2*d)
    ...
    def get_ee_coords(self, m1, m2):
        ...
        theta_3 = self.get_angle(j2z, j2x, j3z, j3x)
        ee_z = j1z - math.sin(theta_2)*self.le
        ee_x = j1x + math.cos(theta_2)*self.le
        return ee_x, ee_z, theta_2, theta_3
